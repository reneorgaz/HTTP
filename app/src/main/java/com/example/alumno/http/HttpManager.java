...
public class HttpManager {
	...
	public String getStrDataByGET() throws IOException   //devuelve un texto
	{
		byte[] bytes = getBytesDataByGET();
		return new String(bytes,"UTF-8");
	}

	public byte[] getBytesDataByGET() throws IOException  //De que manera me voy a comunicar con el servidor
	{
		conn.setRequestMethod("GET");  //dame informacion
		conn.connect();						//me conecto
		int response = conn.getResponseCode();
		Log.d("http", "Response code:" + response);
		if(response==200) {						//ok estoy conectado
			InputStream is = conn.getInputStream();  //obtener
			return readFully(is);			// retorna la informacion requerida
		}
		else
	...
	private byte[] readFully(InputStream inputStream) throws IOException {
		...
		int length = 0;
		while ((length = inputStream.read(buffer)) != -1) {  //leo de la posicion 0 hasta la 1024 por pedazos
